...
void biggies(vector<string> &words,vector<string>::size_type sz){
    ...
    auto fit_it = find_if(words.begin(),words.end(),[sz](const string &s){return s.size() > sz;});
    cout << "more than  " << sz << " count = " << (words.end()-fit_it)<<endl;
    for_each(fit_it,words.end(),[](const string &s){ cout << "s " << s<<endl;});
