...
void biggies(vector<string> &words,vector<string>::size_type sz){
    ...
    auto more_than_sz_count = count_if(words.begin(),words.end(),[sz](const string &s){return s.size() > sz;});

    cout << "more than  " << sz << " count = " << (more_than_sz_count)<<endl;
    for_each(words.end()-more_than_sz_count,words.end(),[](const string &s){cout << s <<endl; });
